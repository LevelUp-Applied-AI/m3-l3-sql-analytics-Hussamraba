...
SELECT
    department_name,
    dept_avg_salary,
    (SELECT avg_salary FROM company_avg) AS company_avg_salary
FROM dept_avg
WHERE dept_avg_salary > (SELECT avg_salary FROM company_avg);
...
('SQL', 'Coursera', 'Intermediate'),
('Data Analysis', 'Udemy', 'Beginner'),
('Project Management', 'PMI', 'Advanced');
